...
```text
POST /shorten {"url": ...}
  API validates the URL (http/https, ≤ 2048 chars) ──invalid──► 422
  → INSERT into Postgres → new row id (e.g. 61)
  → short code = id in base62 ("Z") → return short_url

GET /Z
  decode "Z" → id 61 (invalid characters / out of range ──► 404)
  → Redis GET url:Z ──hit──► 302 redirect
      │ miss (or Redis down → treated as a miss)
      ▼
  → Postgres SELECT id 61 ──not found──► 404
  → write url:Z to Redis (best effort) → 302 redirect
```
...
| Where | What it's for |
|---|---|
| `README.md` (this file) | Run both setups, use the API, operate and troubleshoot it |
| [`documentation.md`](documentation.md) | Decision log, dated: what was done and **why**, every bug found and how it was fixed, alternatives rejected. Sections: Workflow, App, Infra, Bug-fixes, CI, GitOps |
| [`explanations.md`](explanations.md) | Short write-ups of the concepts used: cache-aside, base62, probes, PVCs, Helm, server-side apply, drift, GitHub Actions, … |
| http://localhost:8000/docs | Interactive API reference, generated from the code (can't go out of date) |
| [Runbook](#runbook) | What to check, which commands to run, and how to verify recovery for the three failure scenarios |
| Comments in `scripts/` and `helm/` | Why each non-obvious line is there (e.g. `--force-conflicts`, `rollout status`) |
...
### Assumptions
- **Local only**, as the brief allows: no cloud, no managed database, no real users or domain.
...
- **Links never expire** and can't be edited or deleted — not required by the brief.
- **The reviewer may be on Windows, macOS or Linux** — hence the PowerShell wrappers and
  Helm 3 + 4 support.

### Key decisions
Details and the full reasoning are in each Part's *Design notes* and in `documentation.md`.
...
AI tools (Claude) were used throughout. Nothing was kept on trust — every suggestion was run and
checked live, and several were wrong or incomplete:

| Suggestion | What happened | Outcome |
|---|---|---|
| Point the liveness probe at `/health` too | Would make Kubernetes restart a healthy API during a Postgres outage | **Rejected** → separate `/live` |
| `/health` returns 200 with `"status": "degraded"` in the body | Probes only read status codes, so the readiness probe could never fail | **Changed** → 503 |
| `curl.exe` fixes the PowerShell curl problem | Tested: PowerShell 5.1 still strips the JSON quotes | **Changed** → `Invoke-RestMethod` examples |
| Setup script "verified" | Only tested from Git Bash; from PowerShell `bash` is WSL and can't see `kind` | **Changed** → `.ps1` wrappers, tested from PowerShell |
| Roll back with a script that copies the cluster's values into `values.yaml` | `helm get values` returns only overrides, and it makes the cluster the source of truth | **Rejected** → rollback through git |
| Use the `helm-diff` plugin for drift | Extra install for every tester, and misses `kubectl` edits | **Changed** → `helm template` vs `helm get manifest` + `kubectl diff` |
| Wait for pods with `kubectl wait -l app=api` | During a rolling update it matched the *old* pod; setup exited 52 | **Changed** → `kubectl rollout status` |
| `--force-conflicts` on `helm upgrade` | Needed for Helm 4, but doesn't exist in Helm 3 | **Changed** → only on Helm 4; CI tests both |
| Add `/redoc` to the README | Documented without opening it — it was blank (broken CDN tag) | **Rejected** → removed |
| `docker kill` to simulate a Postgres crash | Docker never auto-restarts containers stopped by hand, so it tested nothing | **Changed** → crash from inside (`pg_ctl stop -m immediate`) |
| 7 linter findings on existing code | 6 were FastAPI patterns or deliberate broad `except`s | **1 accepted**, 6 documented as intentional |
| Store kind's Postgres data in a host folder | Known Postgres ownership errors on Windows bind mounts | **Rejected** → `stop-k8s` keeps the node's disk |
